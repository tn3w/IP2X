...
impl Bin {
    ...
    fn col_off(&self, mid: u32, is_v6: bool, col: u8) -> usize {
    ...
    pub fn str_at(&self, mid: u32, is_v6: bool, col: u8) -> &str {
        let off = self.col_off(mid, is_v6, col);
        let ptr = u32::from_le_bytes(self.mm[off..off + 4].try_into().unwrap()) as usize;
        if ptr == 0 || ptr >= self.mm.len() {
            return "";
        }
        let n = self.mm[ptr] as usize;
        if ptr + 1 + n > self.mm.len() {
            return "";
        }
        let s = std::str::from_utf8(&self.mm[ptr + 1..ptr + 1 + n]).unwrap_or("");
        if s == "-" { "" } else { s }
    }
}
